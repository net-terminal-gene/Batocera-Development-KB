# Dual-Boot GRUB Architecture: Wayland (HD) + X11 (CRT)
...
As of v43, **not all platforms use Wayland**. The Wayland migration is happening for the Steam Deck build exclusively. From the [official download page](https://batocera.org/download):
...
**This distinction is critical for the CRT script install logic** — see the Pre-Flight Check section below.
...
`Batocera-CRT-Script-v43.sh` must detect the active display stack **before** doing anything else. The install path branches based on this result.
...
Batocera-CRT-Script-v43.sh
  ...
  [ Display Stack Check ]
        ...
        └── WAYLAND detected ──► [ Auto-scan for X11 image ]
                                        ...
                                        │         • Option: (2) Download now (direct from Batocera CDN)
...
The CRT script is primarily installed and run **via SSH** from whatever device the user has available — Mac, Windows PC, Linux machine, iPhone, Android phone. See the [installation guide](https://github.com/ZFEbHVUE/Batocera-CRT-Script/blob/main/HowTo_Wired_Or_Wireless_Connection.md) for SSH client options by platform.

This means:
- The script always **executes on Batocera** (Linux), not on the SSH client device
- `dialog` UI (including `dialog --fselect`) runs on Batocera and renders through the SSH terminal — works identically regardless of SSH client OS or device
...
1. Download the X11 Batocera `.img.gz` from [batocera.org/download](https://batocera.org/download) on your PC or Mac (use the "Desktop PC, Laptop, NUC" entry).
2. Transfer it to the Batocera machine using one of the methods below.
3. Run the CRT script via SSH — it will find the file automatically.
...
WinSCP and FileZilla connect over the same network as SSH — no cables, no physical access to the Batocera machine needed. The file lands in `/userdata/` just like any ROM or BIOS file. The CRT script will find it there automatically when it scans for images.
...
**If multiple files are found**, present a numbered list to choose one, or download fresh.
...
The user transfers the file in another window (WinSCP, a second terminal, or plugs in USB) without exiting the script. Selecting **(1) Scan again** re-runs the same `find` scan from the beginning. If the file is now present it is picked up immediately and installation continues — no re-run, no starting over.
...
The loop repeats as many times as needed. **(3) Exit** is the only way out without installing.
...
The script has the direct download URL hardcoded for the current v43 X11 release. The MD5 URL is derived automatically by appending `.md5` — no page scraping, no HTML parsing:

```bash
# Direct download URL hardcoded in the script for the current v43 X11 release.
# Once a major version ships this URL never changes — it contains the build date.
IMAGE_URL="https://mirrors.o2switch.fr/batocera/x86_64/stable/last/batocera-x86_64-43-YYYYMMDD.img.gz"

# MD5 URL derived automatically — same pattern as the beta flow
...
The same URL pair is used whether the image was auto-downloaded by the script or manually transferred by the user — MD5 validation is always a fetch against the mirror, never a scrape of the batocera.org download page.

This single check catches:
- Wrong image (e.g. user accidentally provided a Wayland build instead of X11)
- Wrong version (e.g. v42 image on a v43 system)
- Corrupted download (partial transfer, bad USB, etc.)
- Tampered or unofficial image

If the MD5 does not match the official X11 build for the currently running Batocera version, **the script refuses to proceed** and tells the user exactly why.
...
**The MD5 is the definitive check.** Steps 2 and 3 are safety nets, but matching the official MD5 from the Batocera mirror is the only way to confirm the image is genuinely the correct X11 build — nothing else can confirm that.
...
- **(2) Keep** — file is left untouched. Useful if the user plans to run RESTORE and reinstall later without downloading again.
...
**Space estimate:** ~4.4GB Wayland + ~4.4GB CRT = ~8.8GB of 10GB used. Confirmed feasible on a fresh v43 install which leaves ~5.7GB free. X11 kernel and rufomaculata are always extracted separately — sharing them is not safe since build dates may differ.

**Why a separate X11 kernel?** Kernel modules live inside the squashfs at `/lib/modules/<kernel-version>/`. If the X11 and Wayland builds use different kernel versions (possible across beta build dates), booting the X11 squashfs with the Wayland kernel causes module version mismatches. Using the kernel from the same image as the X11 squashfs guarantees compatibility.
...
Note: All GRUB paths are relative to the VFAT partition root. `/boot/crt/linux` on the running OS = `/crt/linux` in grub.cfg. The CRT entry uses the X11 kernel extracted to `/boot/crt/linux` — not the shared Wayland kernel — to guarantee kernel/module version compatibility with the X11 squashfs.
...
The CRT script uses **Option B**: download the official Batocera X11 `.img.gz` directly from the Batocera CDN using a hardcoded URL for the current v43 release, extract only the `batocera` squashfs from the boot partition of that image, place it at `/boot/crt/batocera`, then delete the downloaded image.

The script has `IMAGE_URL` hardcoded to the direct CDN/mirror URL for the current v43 X11 release. Once a major version ships, that URL never changes — it contains the build date in the filename. The MD5 URL is derived automatically by appending `.md5` to the image URL. No batocera.org page scraping is required.

- Source: official Batocera servers — the image is unmodified upstream content
...
Both locations are checked independently so the user knows exactly which partition is the problem.
...
This works identically over SSH from any client device (Mac, PC, phone). The user always knows exactly how far along the download is and how long remains.
...
- `(2)` drops the user into the same "not found" retry loop described above — the script stays open

**Prerequisite:** An official X11 build of Batocera v43 for x86_64 must exist and be hosted. This is a build/release decision for the upstream project, not a scripting problem.
...
The existing switcher handles config backup/restore and reboot. For dual-boot GRUB there are two new steps in each switch direction: `grub.cfg` default entry change, and overlay save for CRT.

### Switching CRT → HD (Wayland)

1. User opens Mode Switcher from CRT Tools menu → selects **Switch to HD Mode**
2. **NEW:** Calls `batocera-save-crt-overlay` — snapshots current CRT state to `/boot/crt/overlay`
...
`Batocera-CRT-Script-v43.sh RESTORE` performs a full rollback to Wayland-only:
...
Batocera also ships `syslinux.cfg` for non-EFI machines. The CRT script modifies both configs when installing:
...
Note: The X11 kernel is placed at `/boot/crt/linux` — it is a separate copy extracted from the X11 image, not the shared Wayland kernel. Both kernels coexist on the same VFAT partition.
